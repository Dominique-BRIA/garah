...
% =============================================================================
%  ⚠️  DEUX CHOSES A SAVOIR AVANT DE COMPILER
% =============================================================================
%
%  1. LE DOSSIER captures/ DOIT ETRE TELEVERSE AVEC CE FICHIER.
%
%     Cinq figures sont de VRAIES captures de l application. Elles vivent dans
%     docs/captures/ a cote de ce .tex. Sur Overleaf, glissez le dossier entier
%     dans le projet — sans lui, la compilation s arrete sur « File not found ».
%
%     Les autres figures sont des illustrations dessinees en TikZ : elles ne
%     demandent aucun fichier.
%
%  2. SI OVERLEAF DIT « YOUR COMPILE TIMED OUT » : LISEZ CECI.
% =============================================================================
%
...
% --- Une capture RÉELLE de l'application ------------------------------------
%
% #1 = fichier (sans extension)   #2 = repères TikZ, en coordonnées 0..1
%
% ⚠️ Les repères sont posés en coordonnées RELATIVES à l'image. Des
%    coordonnées en centimètres se décaleraient dès qu'on change une marge ou
%    la largeur de l'image.
\newcommand{\capture}[2]{%
  \begin{center}
  \begin{tikzpicture}
    \node[anchor=south west,inner sep=0,draw=garahBordure,line width=0.8pt]
      (img) {\includegraphics[width=0.96\linewidth]{captures/#1}};
...
% --- La légende d'une VRAIE capture -----------------------------------------
%
% ⚠️ Elle ne dit PAS « illustration pédagogique » : celle-ci en est une vraie,
%    et le lecteur doit pouvoir faire la différence d'un coup d'œil.
\newcommand{\legendeCapture}[2]{%
  ...
  \begin{minipage}{0.93\linewidth}
    ...
    \textbf{Figure~#1 --- } #2
    \par\vspace{2pt}
    \textcolor{garahSlogan}{\scriptsize\bfseries Capture réelle de
    l'application.}
  \end{minipage}
  \end{center}
  \vspace{6pt}
...
\begin{attention}
Ce manuel contient \textbf{deux sortes de figures}, et vous devez pouvoir les
distinguer d'un coup d'œil.

\medskip
\textbf{Les captures réelles} portent la mention verte
\textcolor{garahSlogan}{\bfseries « Capture réelle de l'application »}. Ce
sont de véritables images de la plateforme en fonctionnement.

\medskip
\textbf{Les illustrations pédagogiques} portent une mention grise. Elles
reproduisent fidèlement la \emph{disposition} d'un écran — l'ordre des menus,
la place des boutons, les libellés — ou représentent un \emph{enchaînement
d'états}, ce qu'aucune capture ne saurait montrer. Ce ne sont pas des photos
de l'application.
...
\legendeCapture{2.1}{L'accueil de la boutique.
\num{1} le bouton « S'inscrire », en haut à droite.
\num{2} la marque et son slogan, qui ramènent toujours à l'accueil.
\num{3} les puces de catégories : un clic ouvre le catalogue déjà filtré.}
...
\begin{astuce}
Les puces de catégories sont visibles sur la figure~2.1, repère \num{3}. Un
seul clic suffit : le catalogue s'ouvre \textbf{déjà filtré} sur la catégorie
choisie.
\end{astuce}
...
\legendeCapture{2.11}{Le catalogue complet.
\num{1} la barre de recherche.
\num{2} le nombre d'articles trouvés, sous le titre.
\num{3} l'étiquette « PLUS QUE 5 » : le stock devient bas.}
...
\legendeCapture{2.3}{La fiche d'un article, avec sa grille de paliers.
\num{1} le palier qui s'applique à la quantité choisie, encadré.
\num{2} les paliers suivants, atteignables en commandant davantage.
\num{3} ce qu'il manque pour descendre d'un palier.
\num{4} les déclinaisons ; les barrées sont épuisées.
\num{5} la quantité, et le stock disponible juste dessous.
\num{6} le sous-total --- \emph{hors} frais d'acheminement.
\num{7} « Ajouter au panier ».}
...
\legendeCapture{2.10}{La page « Suivre un colis ».
\num{1} le champ du numéro de suivi.
\num{2} le bouton qui lance la recherche.
...
\capture{connexion-admin}{%
  \node at (0.318,0.399) {\num{1}};
  \node at (0.318,0.291) {\num{2}};
  \node at (0.318,0.204) {\num{3}};
  \node at (0.91,0.952) {\num{4}};
}
\legendeCapture{3.6}{L'écran de connexion du back-office.
\num{1} votre adresse e-mail.
\num{2} votre mot de passe.
\num{3} « Se connecter ».
\num{4} la bascule clair / sombre --- elle est disponible \emph{avant} même
d'être connecté.}
